  ...
  for (let attempt = 1; attempt <= 10; attempt += 1) {
    ...
    for (const row of rows) {
      ...
      const rowMeta = await row.evaluate((el) => {
        const header = el.querySelector('[role="rowheader"]') || el;
        return {
          ariaLabel: header.getAttribute('aria-label') || '',
          objectType: header.getAttribute('data-objecttype') || '',
          id: header.getAttribute('id') || '',
          text: (header.textContent || '').trim(),
        };
      }).catch(() => ({ ariaLabel: '', objectType: '', id: '', text: rowText }));
      const isAdsetRow = rowMeta.ariaLabel === '광고 세트' || rowMeta.objectType === 'CAMPAIGN';
      const isAdRow = rowMeta.ariaLabel === '광고' || rowMeta.objectType === 'ADGROUP';
      const isAdsetCopy = isAdsetRow && ((rowText.includes('광고세트') && rowText.includes('사본')) || /리타겟\s*\d+번\s*광고세트/.test(rowText));
      const isAdCopy = isAdRow && (rowText.includes('새 판매 광고') || rowText.includes('광고 - 사본') || rowText.includes('광고명') || /f_i_o_l_\d{4}_\d+/.test(rowText));
      console.log('[DEBUG] 순차 row 판정:', {
